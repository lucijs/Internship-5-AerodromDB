...
select * from airports  a where (SELECT sqrt(power(c.Latitude::float -43.5147::float,2)+power(c.Latitude::float -16.4435::float,2)) FROM Cities c WHERE c.CityId = a.CityId)<1500;

select Price*0.8 from Tickets t where  (select count(*) from flights f where f.FlightId=t.FlightId)<20;

update  Airplanes
	set StateId = 4
	where (select count(*) from flights f where airplaneId = f.AirplaneId and Departure<'2023-12-11')=0 and NOW()-Produced<interval(hour := 20*24*365.25);